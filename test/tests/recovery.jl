...
wts_kry = window_quadrature_weights(pts, win, solver=KrylovSolver(hm_pre, SincKernel, 1e-8))[2]
...
wts_kry  = window_quadrature_weights(pts, win, solver=KrylovSolver(chol_pre, SincKernel, 1e-8))[2]
rec_kry  = abs2.(F*wts_kry)
@test maximum(rec_kry[ix_out]) < 1e-15
